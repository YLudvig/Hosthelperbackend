...
public class RemoteController {
    ...
    public ResponseEntity<Map<String, String>> updateRemote(@PathVariable UUID remoteId, @RequestBody RemoteDTO dto){
        remoteService.updateRemote(remoteId, dto);
        return ResponseEntity.ok(Map.of("message", "Remote updated successfully!"));
    }

    // Endpoint for user to delete a remote
    @DeleteMapping("/delete/{remoteId}")
    public ResponseEntity<Map<String, String>> deleteRemote(@PathVariable UUID remoteId){
        remoteService.removeRemote(remoteId);
        return ResponseEntity.ok(Map.of("message","Remote removed!"));
    }
